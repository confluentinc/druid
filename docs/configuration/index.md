...
|`druid.processing.numThreadPools`|The number of independent processing thread pools (each with its own task queue) to split `druid.processing.numThreads` across. The processing threads are divided as evenly as possible among the pools. Each pool has its own queue lock, so increasing this reduces contention on a single queue lock under very high task rates (for example, one task per segment scan on a Historical serving many small segments). The trade-off is that priority ordering becomes per-pool rather than global, so leave this at `1` unless the processing queue lock is a measured bottleneck. Capped at `druid.processing.numThreads` (you cannot have more pools than threads).|`1` (a single pool; identical to the previous behavior)|
